  ...
  try {
    ...
    switch (action) {
      case 'add': {
        ...
        if (adminConfig.SourceConfig.some((s: any) => s.key === key)) {
      ...
      case 'disable': {
        ...
        const entry = adminConfig.SourceConfig.find((s: any) => s.key === key);
        if (!entry)
      ...
      case 'delete': {
        ...
        if (!key)
          return NextResponse.json({ error: '缺少 key 参数' }, { status: 400 });
        const idx = adminConfig.SourceConfig.findIndex((s: any) => s.key === key);
        if (idx === -1)
          return NextResponse.json({ error: '源不存在' }, { status: 404 });
        ...
        if (adminConfig.UserConfig.Tags) {
          adminConfig.UserConfig.Tags.forEach((tag: any) => {
            if (tag.enabledApis) {
              tag.enabledApis = tag.enabledApis.filter((api: any) => api !== key);
            }
          });
        }
      ...
      case 'batch_delete': {
        ...
        if (keysToDelete.length > 0) {
          ...
          if (adminConfig.UserConfig.Tags) {
            adminConfig.UserConfig.Tags.forEach((tag: any) => {
              if (tag.enabledApis) {
                tag.enabledApis = tag.enabledApis.filter((api: any) => !keysToDelete.includes(api));
              }
            });
          }
      ...
      case 'sort': {
        ...
        const map = new Map(adminConfig.SourceConfig.map((s: any) => [s.key, s]));
        ...
        // 未在 order 中的保持原顺序
        adminConfig.SourceConfig.forEach((item: any) => {
