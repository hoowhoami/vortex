...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      case 'add':
        // 检查是否已存在相同的 key
        if (config.LiveConfig.some((l: any) => l.key === key)) {
      ...
      case 'delete':
        // 删除直播源
        const deleteIndex = config.LiveConfig.findIndex((l: any) => l.key === key);
        if (deleteIndex === -1) {
      ...
      case 'enable':
        // 启用直播源
        const enableSource = config.LiveConfig.find((l: any) => l.key === key);
        if (!enableSource) {
          return NextResponse.json({ error: '直播源不存在' }, { status: 404 });
        }
      ...
      case 'disable':
        // 禁用直播源
        const disableSource = config.LiveConfig.find((l: any) => l.key === key);
        if (!disableSource) {
          return NextResponse.json({ error: '直播源不存在' }, { status: 404 });
        }
      ...
      case 'edit':
        // 编辑直播源
        const editSource = config.LiveConfig.find((l: any) => l.key === key);
        if (!editSource) {
          return NextResponse.json({ error: '直播源不存在' }, { status: 404 });
        }
      ...
      case 'sort':
        ...
        order.forEach((key: any) => {
          const source = config.LiveConfig?.find((l: any) => l.key === key);
          if (source) {
        ...
        config.LiveConfig.forEach((source: any) => {
