    ...
    switch (action) {
      ...
      case 'userGroup': {
        ...
        switch (groupAction) {
          case 'add': {
            // 检查用户组是否已存在
            if (adminConfig.UserConfig.Tags.find((t: any) => t.name === groupName)) {
          ...
          case 'edit': {
            const groupIndex = adminConfig.UserConfig.Tags.findIndex((t: any) => t.name === groupName);
            if (groupIndex === -1) {
          ...
          case 'delete': {
            ...
            adminConfig.UserConfig.Users.forEach((user: any) => {
              if (user.tags && user.tags.includes(groupName)) {
                ...
                // 从用户的tags中移除该用户组
                user.tags = user.tags.filter((tag: any) => tag !== groupName);
      ...
      case 'batchUpdateUserGroups': {
        ...
        if (operatorRole !== 'owner') {
          for (const targetUsername of usernames) {
            const targetUser = adminConfig.UserConfig.Users.find((u: any) => u.username === targetUsername);
